...
- 使用者已主動把 Brownfield Planner 提供的可複製交接文字貼入本 Task；交接文字必須同時包含可開啟的 Planner HTML URL 與該 HTML 的絕對路徑，兩者必須指向同一份架構報告，並包含工作識別碼、顯示名稱、實際工作目錄、Spec 與 Tickets 路徑。任一值缺少、仍是占位符或指向不同報告時停止。此動作即代表使用者已核准報告與執行計畫，不再檢查 HTML 是否具有核准標記，也不再次詢問。平台自動建立 Task 或 Agent 之間自行轉送，不代替這次使用者手動貼入。
...
2. 若實際工作目錄已存在正式 `completion-report.html`，先以 `brownfield-implement` 與實際工作目錄唯讀驗證：通過時顯示既有報告並停止；失敗時回報 `BLOCKED: PREMATURE_COMPLETION_REPORT_EXISTS` 與實際錯誤，不覆寫、不刪除，也不在同一識別碼繼續新需求。`completion-report.pending.html` 只是尚未交付的候選檔，不代表完成；可在全部結案條件成立後繼續驗證及修正。
...
- 派發每張 Brownfield Ticket 的 Developer 前，先確認 `## 原有功能確認` 已恰好預建一份 `共用驗證方法` 與一份修改前實測所得的 `修改前基準`。任一欄缺少、仍是占位符、只寫假設或沒有實際證據時，回報 `BLOCKED: ORIGINAL_BEHAVIOR_BASELINE_MISSING`；不得先修改再補猜基準。
...
- `## 原有功能確認`：`共用驗證方法`、`修改前基準`、`修改後結果`、`原有功能驗收`、`驗收證據`。
...
- 使用者同意延續後，不替使用者建立 Task；固定輸出一段可直接複製貼到新 Task 的完整交接文字，同時包含可開啟的進度頁 URL 與 `progress-report.html` 絕對路徑，兩者必須指向同一工作；Planner HTML 也必須同時附可開啟 URL 與絕對路徑，且兩者指向同一份架構報告。另包含工作識別碼、顯示名稱、工作目錄、Spec、Tickets、目前 Snapshot、未完成項目與必要證據。輸出後不得留在舊 Task 繼續。
- 新需求不得沿用執行 Task，必須重新從 `$milktea-skills-grill-me` 開始。

## 原有功能確認

所有 Tickets 完成並通過各自的驗收與 Review 後，讀取 Planner 報告中的「必須保留功能」，逐票使用已核准 Ticket 內唯一的 `共用驗證方法` 執行修改後驗證。這個欄位同時規定修改前與修改後的完整實測步驟，不得另外建立「修改前方法」與「修改後方法」，也不得在執行階段偷偷換方法。依實際使用方式決定該單一方法的內容，不把所有保留功能都硬塞進瀏覽器驗收：

- UI、網站、桌面介面或其他使用者可互動功能：必須使用 Claude／Codex 當下平台真正提供的原生瀏覽器、Chrome 或電腦操作能力實際操作。
- API、CLI、Worker、排程、資料庫、Migration 或其他沒有使用者介面的功能：依 Planner／Ticket 指定的 API 呼叫、CLI 指令、測試、Job 執行、資料查核或 Migration 驗證完成；不因沒有瀏覽器畫面而阻擋。
- 同時包含 UI 與非 UI 的功能：在同一個 `共用驗證方法` 內列出兩部分的固定步驟並分別保存證據，UI 部分不得被 API 或測試取代。

逐票只原地填寫既有五個唯一欄位：

- `共用驗證方法`：規劃階段已核准、修改前後完全相同的實際步驟與工具；執行階段不得新增第二種方法。
- `修改前基準`：修改前依共用方法取得的實際值、畫面、輸出或行為，必須可追溯到工具、環境、版本與證據。
- `修改後結果`：修改後再次依同一共用方法取得的實際值、畫面、輸出或行為。
- `原有功能驗收`：只有修改後結果符合核准保留條件時才原地填為精確的「通過」。
- `驗收證據`：同時引用修改前基準與修改後結果的實際命令、退出碼、網址、畫面、截圖、資料查核或 Job 輸出。

「應該相同」、「依程式判斷」、「沒有修改到」、「無法確認」或任何沒有實測來源的文字都不是驗收證據。無法再次執行共用方法、缺少修改前基準或結果不能比較時，保持未通過並回報阻擋；不得用另一種較方便的方法替代，也不得宣告原有功能正常。
...
1. 每張 Ticket 的 `## 前端實際操作驗收` 必須明確標示「適用」或「不適用」及判定依據；UI／可互動項目不能因沒有既有 E2E 就判定不適用，純 API、CLI、Worker、排程、資料庫或 Migration 則應判定不適用並引用其對應驗證證據。
...
7. 最終結果逐項原地更新受影響 Ticket 的既有唯一欄位；Planner 要求用相同方法比較的原有功能必須保存修改前後證據。全部適用項目與必須保留功能通過前，不得進入結案報告階段。

## 完成規則

- 所有核准 Tickets 與驗收條件均有可重現證據。
- Planner 列出的所有必須保留功能均已依指定方式確認正常。
- 所有 UI／可互動需求與必須保留功能已由 Claude／Codex 的原生瀏覽器控制能力實際操作並通過；所有非 UI 項目已依對應 API、CLI、Worker、資料庫或其他指定方法驗證通過，並有不適用瀏覽器的實際判定依據。
- 必跑指令成功；失敗或未執行項目已明示。
...
單張 Ticket 完成、單次 Review、修正、複驗或原有功能單項確認時，禁止產生或更新 `completion-report.pending.html` 與正式 `completion-report.html`；`progress-report.html` 仍須隨 Ticket 更新並經 SSE 推送。任一 Ticket 或必須保留功能未通過時只回報阻擋，待全部完成後才進入結案候選檔階段。

產生前再次確認正式 `completion-report.html` 不存在；已存在時停止，不能覆寫。共用 HTML 報告技能不存在時回報 `BLOCKED: HTML_REPORT_SKILL_UNAVAILABLE`。先產生或更新候選 `completion-report.pending.html`，並以 `brownfield-implement`、實際工作目錄及候選路徑驗證；失敗時只修正候選檔，正式檔必須保持不存在，進度頁也不得顯示已完成。候選驗證通過後，由驗證器在同一檔案系統內執行不覆寫的原子發布，建立正式 `completion-report.html` 並移除候選檔；正式目標若在發布前出現就停止，不覆寫。

原子發布成功後不可立即停止 SSE。先對目前進度服務執行 `GET /snapshot`，利用該端點強制重新讀取工作目錄；確認回傳 JSON 的 `報告狀態` 精確等於 `completed`，再唯讀確認磁碟上的 `progress-report.html` 最後快照已顯示完成狀態，且包含指向本工作正式 `completion-report.html` 的連結。兩項都成立後，才能宣稱重構完整交付並停止 SSE 服務。任一項不成立時回報 `BLOCKED: FINAL_PROGRESS_SNAPSHOT_UNCONFIRMED` 與實際回應或檔案缺口，不得顯示完成訊息，也不得先停止服務。
