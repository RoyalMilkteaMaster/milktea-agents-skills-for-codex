...
最後輸出每張 Ticket 的 execution environment、狀態、角色後端、實際模型、實際 `model_reasoning_effort` 或模型預設、兩位 Reviewer 的 `review_engine`、OCR Delegation 證據或降級原因、測試證據、兩份 Review 結論、共識、未解風險與版本識別。
